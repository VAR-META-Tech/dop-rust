...
import { hashPasswordString, getRandomBytesCall } from '../core/encrypts.js';
export const encryptRouter = express.Router();
encryptRouter.get('/getRandomBytes', async (req, res) => {
    const { length } = req.query;
    try {
        const bytes = getRandomBytesCall(10);
        res.json({ bytes });
    }
    catch (err) {
...
encryptRouter.get('/getHashPassword', async (req, res) => {
    const { password, salt, iterations } = req.query;
    try {
        const hs = await hashPasswordString(password || '', salt || '', parseInt(iterations || '0'));
        res.json({ hs });
    }
    catch (err) {
