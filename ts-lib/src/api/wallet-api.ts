...
import {
  ...
  generateMnemonic,
  getWalletById,
  getWalletShareableViewingKeyById,
} from "../core/wallet.js";
...
import { Chain } from "dop-sharedmodels-v3";
...
walletRouter.get("/validate/dop-address", (req, res) => {
  const { address } = req.query;
  try {
    const valid = validateDopAddress(address as string);
    res.json({ valid });
  } catch (err) {
...
walletRouter.get("/validate/eth-address", (req, res) => {
  const { address } = req.query;
  try {
    const valid = validateEthAddress(address as string);
    res.json({ valid });
  } catch (err) {
...
walletRouter.get("/assert/eth-address", (req, res) => {
  const { address } = req.query;
  try {
    assertValidEthAddress(address as string); // Throws if invalid
...
walletRouter.get("/assert/dop-address", (req, res) => {
  const { address } = req.query;
  try {
    assertValidDopAddress(address as string); // Throws if invalid
    res.sendStatus(204); // No content = valid
  } catch (err) {
